...
int move_straight(float distance){
	...
	while (SensorValue(Power_Switch) == Power_Switch_ON && ball_caught == 0 && sqrt(pow(odom.Y-current_Y,2)+pow(odom.X-current_X,2))<distance && boundary_avoidance()){
		moveMotor(1,1,'f',40);
		motor[roller] = 127;
		motor[servo] = -100;
...
int pan_by_degree(float degree, char dir){
	...
	while (SensorValue(Power_Switch) == Power_Switch_ON && ball_caught == 0 && fabs(odom.heading-current_heading)<degree && boundary_avoidance()){
...
int ball_return(void){
	...
	while (pan_to_heading(90)!=1 && boundary_avoidance() ){
		pan_to_heading(90);
	}

	pan_by_degree(22.5,'l');
...
int boundary_avoidance(){
	int prev_heading = compass(); //Store heading when hit boundary

	if (SensorValue(leftLF) == 0){	//Checks left line follower
		moving_back_t = nSysTime;
		while (nSysTime - moving_back_t < 1500  && ball_caught==0 && SensorValue(Power_Switch) == Power_Switch_ON){ //Move back at 0.3 speed for 1.5 seconds
			moveMotor(0.3,0.3,'b',0);
		}

		if (prev_heading == 0 || prev_heading == 45 || prev_heading == 90 || prev_heading == 135 || prev_heading == 180) {	//If robot is facing south direction, rotate back to South)
			pan_to_heading(90);
		}

		else if(prev_heading == 225 || prev_heading == 270 || prev_heading == 315){	//If robot facing North direction, rotate back to North
			pan_to_heading(270);
		}
...
int pan_to_heading(float heading){
	...
	while  (array[pointer] != compass() && SensorValue(Power_Switch) == Power_Switch_ON && boundary_avoidance()){
		...
		if (compass() - array[pointer] >= 0 && compass()-array[pointer] <=180 ){
			minimum_t = nSysTime;
			while (nSysTime - minimum_t < 200){ //Move at least for 1 second for angles close to the landmarks
